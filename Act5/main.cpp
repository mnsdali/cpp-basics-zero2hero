...
public:
    ...
    Article(string n, double p, bool r = false) : nom(n), prix(p), reduit(r) {}
};
...
public:
    ...
    void scanner(Caddie caddie) {
        total = 0;
        for (const Article& article : caddie.articles) {
            total += article.prix;
            if (article.reduit) {
                total *= 0.9; // Apply 10% discount for reduced items
            }
        }
    }

    void afficher() {
        cout << "Total à payer : " << total << " euros";
    }
};
...
int main() {
    ...
    Article roman("Les delices de Sophie", 16.50, true);
